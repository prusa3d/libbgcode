...
namespace {
// This namespace contains copies of boost::beast::detail::base64 routines,
// so we don't depend on more boost components than we need.

char const* base64_get_alphabet()
{
...
signed char const* base64_get_inverse()
{
...
std::size_t constexpr base64_decoded_size(std::size_t n)
{
    return n / 4 * 3;
}

std::pair<std::size_t, std::size_t> base64_decode(void* dest, char const* src, std::size_t len)
{
    ...
    auto const inverse = base64_get_inverse();
...
std::size_t base64_encode(void* dest, void const* src, std::size_t len)
{
    char*      out = static_cast<char*>(dest);
    char const* in = static_cast<char const*>(src);
    auto const tab = base64_get_alphabet();
    ...
    if (!parser.parse([&](GCodeReader& r, const GCodeReader::GCodeLine& line) {
        ...
        if (!reading_thumbnail.has_value()) {
            ...
            if (thumbnail_end) {
                ...
                std::vector<std::byte> decoded(base64_decoded_size(thumbnail.data.size()));
                auto thumbnail_buf = reinterpret_cast<const char *>(thumbnail.data.data());
                decoded.resize(base64_decode(decoded.data(), thumbnail_buf, thumbnail.data.size()).first);
...
BGCODE_CONVERT_EXPORT EResult from_binary_to_ascii(FILE& src_file, FILE& dst_file, bool verify_checksum)
{
    ...
    //
    // convert file metadata block, if present
    //
    ...
    //
    // convert thumbnail blocks, if present
    //
    ...
    while ((EBlockType)block_header.type == EBlockType::Thumbnail) {
        ...
        std::string encoded;
        encoded.resize(base64_encoded_size(thumbnail_block.data.size()));
        encoded.resize(base64_encode((void*)encoded.data(), (const void*)thumbnail_block.data.data(), thumbnail_block.data.size()));
        std::string format;
